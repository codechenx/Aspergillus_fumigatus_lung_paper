...
for i in sorted(list(set(feature_table["Freq"])),reverse=True):
    ...
    if mean_cv[i]["MCC"] > max_mcc :
        ...
        plot_model(tuned_model, 'auc', save=True)
        plot_model(tuned_model, 'pr', save=True)
        plot_model(tuned_model, 'class_report', save=True)
        plot_model(tuned_model, 'confusion_matrix', save=True)
        plot_model(tuned_model, 'error', save=True)
# %%
